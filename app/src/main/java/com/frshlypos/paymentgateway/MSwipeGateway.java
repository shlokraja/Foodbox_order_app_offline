...
public class MSwipeGateway implements IPaymentGateway {
    ...
    public MSwipeGateway(MainActivity myActivity) {
    ...
    public boolean connectToDevice(boolean duringTransaction) {
        ...
        if (!BluetoothAdapter.getDefaultAdapter().isEnabled()) {
            ...
            ((MainActivity) myContext).startActivityForResult(enableBtIntent, REQUEST_ENABLE_BT);
            ...
            SharedPreferences sharedPref = ((MainActivity) myContext).getPreferences(Context.MODE_PRIVATE);
            ...
            if (pairedDevicesFound.size() > 1) {
                ...
                /*((MainActivity)myContext).myWebView.post(new Runnable() {
                    @Override
                    public void run() {
                        ((MainActivity)myContext).myWebView.evaluateJavascript("$('#settingsModal #status_text').empty(); $('#settingsModal #status_text').text('Sorry, multiple matched devices found.');", null);
                    }
                });*/
                ((MainActivity) myContext).setSettingsStatusText("Sorry, multiple matched devices found.");
                ...
                /*((MainActivity)myContext).myWebView.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        ((MainActivity)myContext).myWebView.evaluateJavascript("$('#settingsModal #status_text').empty(); $('#settingsModal #status_text').text('Exact match to device id. Starting connection.');", null);
                    }
                });*/
                ((MainActivity) myContext).setSettingsStatusText("Exact match to device id. Starting connection.");
                ...
                /*((MainActivity)myContext).myWebView.post(new Runnable() {
                    @Override
                    public void run() {
                        ((MainActivity)myContext).myWebView.evaluateJavascript("$('#settingsModal #status_text').empty(); $('#settingsModal #status_text').text('Sorry, not matched devices found.');", null);
                    }
                });*/
                ((MainActivity) myContext).setSettingsStatusText("Sorry, not matched devices found.");
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            try {
                ...
                if (status.equalsIgnoreCase("False")) {
                    ...
                    ((MainActivity) myContext).setSettingsStatusText(ErrMsg);
                    //((MainActivity)myContext).myWebView.evaluateJavascript("$('#settingsModal #status_text').empty(); $('#settingsModal #status_text').text('"+ ErrMsg +"');", null);
                } else {
                    ...
                    ((MainActivity) myContext).showSummary(saleAmount, saleCount, bankSummaryDate);
                    /*((MainActivity)myContext).myWebView.evaluateJavascript("showBankSummary("+saleAmount+","+saleCount+","+bankSummaryDate+")",
                            null);*/
                }
            } catch (Exception ex) {
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            try {
                ...
                if (status.equalsIgnoreCase("false")) {
                    ...
                    ((MainActivity) myContext).setSettingsStatusText(displayMsg);
                    //((MainActivity)myContext).myWebView.evaluateJavascript("$('#settingsModal #status_text').empty(); $('#settingsModal #status_text').text('"+ displayMsg + "');", null);
